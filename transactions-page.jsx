// Transactions page — full ledger with filters, segments, search, and detail drawer.
const { useState: useStateT, useMemo: useMemoT } = React;
const { fmt: fmtT } = window.KauriComponents;
...
function TransactionsPage() {
  ...
  const [seg, setSeg] = useStateT("tous");
  const [channel, setChannel] = useStateT("tous");
  const [agent, setAgent] = useStateT("tous");
  const [period, setPeriod] = useStateT("7j");
  const [selected, setSelected] = useStateT(null);

  const agents   = useMemoT(() => ["tous", ...Array.from(new Set(TX_FULL.map(t => t.agent)))], []);
  const channels = useMemoT(() => ["tous", ...Array.from(new Set(TX_FULL.map(t => t.channel)))], []);

  const filtered = useMemoT(() => TX_FULL.filter(t => {
  ...
  return (
    <div className="tx-page">
      <div className="topbar">
        <div>
          <div className="crumbs"><strong>Kauri</strong> &nbsp;/&nbsp; Transactions</div>
          <h1 className="h1">Transactions</h1>
          <div className="h1-sub">{TX_FULL.length} mouvements · 2 TPE actifs · synchronisé il y a 1 min</div>
        </div>
        <div className="topbar-right">
          <div className="search" style={{ width: 280 }}>
            <I.Search /><input placeholder="Client, ID, TPE, référence…" value={q} onChange={e => setQ(e.target.value)}/>
          </div>
          <div className="seg">
      ...
      <div className="card">
        ...
        <div className="bulk-bar" style={{ background: "var(--surface-2)" }}>
          ...
          <span style={{ color: "var(--pos)", fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>+{fmtT(totalIn)} FCFA</span>
          <span style={{ color: "var(--neg)", fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>−{fmtT(totalOut)} FCFA</span>
          <span style={{ color: "var(--ink-3)" }}>·</span>
          <span style={{ fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>Net {net >= 0 ? "+" : "−"}{fmtT(Math.abs(net))} FCFA</span>
          <div style={{ flex: 1 }}/>
          ...
          <div style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 28, fontWeight: 600, letterSpacing: "-0.025em", fontVariantNumeric: "tabular-nums",
                ...
                {tx.type === "in" ? "+" : "−"}{fmtT(tx.amount)}<span style={{ fontSize: 13, marginLeft: 6, fontWeight: 500, opacity: 0.8 }}>FCFA</span>
              </div>
            </div>
          </div>
