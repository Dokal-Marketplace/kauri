// Settings page — institution config, agency, TPE, security, notifications.
const { useState: useStateS } = React;

function SettingsPage() {
  const [section, setSection] = useStateS("agence");
  const [orgName, setOrgName] = useStateS("Kauri Microfinance");
  const [currency, setCurrency] = useStateS("FCFA (XOF)");
  const [twoFA, setTwoFA] = useStateS(true);
  const [autoLock, setAutoLock] = useStateS(5);
  const [notif, setNotif] = useStateS({ deposit: true, withdraw: true, lowBattery: true, syncFail: true, weekly: false });
  const [bgSync, setBgSync] = useStateS(true);
  const [offlineDays, setOfflineDays] = useStateS(3);
  ...
  return (
    <div className="settings-page">
      <div className="topbar">
        <div>
          <div className="crumbs"><strong>Kauri</strong> &nbsp;/&nbsp; Paramètres</div>
          <h1 className="h1">Paramètres</h1>
          <div className="h1-sub">Configuration de l’institution, des terminaux et de la sécurité</div>
        </div>
          ...
          {section === "agence" && (
            <>
              <SetCard title="Identité de l’institution" sub="Visible dans les reçus, exports et l’en-tête de l’app.">
                ...
                <Field label="Devise"><select className="input" value={currency} onChange={e => setCurrency(e.target.value)}><option>FCFA (XOF)</option><option>EUR</option><option>USD</option></select></Field>
                <Field label="Fuseau horaire"><select className="input" defaultValue="GMT+0 · Ouagadougou"><option>GMT+0 · Ouagadougou</option><option>GMT+0 · Bamako</option><option>GMT+1 · Lagos</option></select></Field>
                <Field label="Logo (PNG/SVG)"><div className="upload"><div className="brand-mark" style={{ width: 40, height: 40 }}><I.KauriDrop stroke="white"/></div><div><div style={{ fontWeight: 550, fontSize: 13 }}>kauri-logo.svg</div><div className="cell-sub">3,2 Ko · remplacer</div></div><button className="btn sm" style={{ marginLeft: "auto" }}>Choisir…</button></div></Field>
              </SetCard>
              ...
              <SetCard title="Agences" sub="3 agences actives · ajoutez ou archivez selon votre couverture.">
                <table className="data-table" style={{ minWidth: "auto" }}>
                  ...
                  <tbody>
                    {[
                      { n: "Bobo-Dioulasso", c: "BBO", a: 3, s: "actif" },
                      { n: "Banfora",        c: "BNF", a: 2, s: "actif" },
                      { n: "Hounde",         c: "HND", a: 1, s: "actif" },
                    ].map(b => (
                      <tr key={b.c}><td style={{ fontWeight: 550 }}>{b.n}</td><td className="cell-sub" style={{ fontFamily: "var(--font-mono)" }}>{b.c}</td><td>{b.a}</td><td><span className="tag actif">Actif</span></td><td><button className="btn ghost sm" style={{padding:4}}><I.More size={14}/></button></td></tr>
                    ))}
                  </tbody>
                </table>
            ...
            <>
              ...
              <SetCard title="Mises à jour TPE" sub="Version 4.2.1 disponible. 5 / 6 terminaux à jour.">
                <div className="upload">
                  ...
                  <div><div style={{ fontWeight: 550, fontSize: 13 }}>Kauri TPE 4.2.1 · 4 mai 2026</div><div className="cell-sub">Corrige le délai de sync sur réseau 2G.</div></div>
          ...
          {section === "securite" && (
            <>
              <SetCard title="Authentification" sub="Connexion des agents et accès au tableau d’administration.">
                ...
                <Field label="Politique de PIN"><select className="input" defaultValue="6 chiffres · rotation 90 j"><option>4 chiffres · rotation 180 j</option><option>6 chiffres · rotation 90 j</option><option>6 chiffres + biométrie</option></select></Field>
              </SetCard>
              <SetCard title="Audit & journalisation" sub="Conservation des journaux d’événements.">
                <Field label="Durée de conservation"><select className="input" defaultValue="36 mois"><option>12 mois</option><option>24 mois</option><option>36 mois</option><option>60 mois</option></select></Field>
            ...
            <>
              ...
              <SetCard title="Méthode de paiement">
                <div className="upload">
                  ...
                  <div><div style={{ fontWeight: 550, fontSize: 13 }}>Mobile Money · Orange BF</div><div className="cell-sub">+226 70 •• •• 22</div></div>
            ...
            <SetCard title="Intégrations" sub="Connectez Kauri à vos outils existants.">
              {[
                ...
                <div key={it.n} className="upload">
                  ...
                  <div><div style={{ fontWeight: 550, fontSize: 13 }}>{it.n}</div><div className="cell-sub">{it.d}</div></div>
