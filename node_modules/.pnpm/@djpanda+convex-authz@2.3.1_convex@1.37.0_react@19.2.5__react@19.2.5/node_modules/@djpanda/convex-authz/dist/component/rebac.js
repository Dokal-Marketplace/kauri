        ...
        while (queue.length > 0) {
            ...
            for (const rule of currentRules) {
                ...
                const parentRelations = await ctx.db
                    .query("relationships")
                    .withIndex("by_tenant_object_relation", (q) => q
                    .eq("tenantId", args.tenantId)
                    .eq("objectType", current.objectType)
                    .eq("objectId", current.objectId)
                ...
                const parents = parentRelations.filter((r) => r.subjectType === rule.through);
...
export const listAccessibleObjects = query({
    ...
    handler: async (ctx, args) => {
        ...
        const directRelations = await ctx.db
            ...
            .eq("subjectId", args.subjectId))
            .take(1000);
        const directObjects = directRelations.filter((r) => r.relation === args.relation && r.objectType === args.objectType);
...
export const listUsersWithAccess = query({
    ...
    handler: async (ctx, args) => {
        ...
        const directRelations = await ctx.db
            ...
            .eq("objectId", args.objectId))
            .take(1000);
        const directUsers = directRelations.filter((r) => r.relation === args.relation && r.subjectType === "user");
