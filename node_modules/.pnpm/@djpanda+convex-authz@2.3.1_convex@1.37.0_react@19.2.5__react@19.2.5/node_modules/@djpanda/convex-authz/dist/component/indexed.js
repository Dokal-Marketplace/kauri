...
export const checkPermissionsFast = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.permissions.length > MAX_BULK_PERMISSIONS) {
            throw new Error(`permissions must not exceed ${MAX_BULK_PERMISSIONS} items (got ${args.permissions.length})`);
        }
        const scopeKey = args.objectType && args.objectId
            ? `${args.objectType}:${args.objectId}`
            : "global";
        ...
        const rows = await ctx.db
            .query("effectivePermissions")
            .withIndex("by_tenant_user_scope", (q) => q.eq("tenantId", args.tenantId).eq("userId", args.userId).eq("scopeKey", scopeKey))
            .take(1000);
        const allowedPerms = [];
        const deniedPerms = [];
        for (const row of rows) {
            if (row.expiresAt && row.expiresAt < now)
                continue;
            if (row.effect === "allow")
                allowedPerms.push(row.permission);
            else if (row.effect === "deny")
                deniedPerms.push(row.permission);
        }
    ...
    handler: async (ctx, args) => {
        ...
        if (args.scopeKey) {
            permissions = await ctx.db
                .query("effectivePermissions")
                .withIndex("by_tenant_user_scope", (q) => q.eq("tenantId", args.tenantId).eq("userId", args.userId).eq("scopeKey", args.scopeKey))
                .take(1000);
        }
        else {
            permissions = await ctx.db
...
export const cleanupExpired = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        while (true) {
            ...
            if (permissions.length < CLEANUP_BATCH || !deletedAny)
                break;
        }
        ...
        while (true) {
            ...
            if (roles.length < CLEANUP_BATCH || !deletedAny)
                break;
        }
