...
async function offboardUserImpl(ctx, args) {
    ...
    if (fullDeprovision && removeRels) {
        const relationships = await ctx.db
            .query("relationships")
            .withIndex("by_tenant_subject", (q) => q
        ...
        const effectiveRels = await ctx.db
            ...
            .withIndex("by_tenant_subject", (q) => q.eq("tenantId", args.tenantId).eq("subjectKey", userSubjectKey))
            .take(4000);
    ...
    for (const r of effectiveRoles) {
        if (scopeKey !== null && r.scopeKey !== scopeKey)
            continue;
        await ctx.db.delete(r._id);
    ...
    for (const p of effectivePerms) {
        if (scopeKey !== null && p.scopeKey !== scopeKey)
            continue;
...
export const deprovisionUser = mutation({
    ...
    handler: async (ctx, args) => offboardUserImpl(ctx, {
        tenantId: args.tenantId,
        userId: args.userId,
...
export const cleanupExpired = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        while (true) {
            ...
            if (roleAssignments.length < BATCH_SIZE || !deletedAny)
                break;
        }
        ...
        while (true) {
            ...
            if (overrides.length < BATCH_SIZE || !deletedAny)
                break;
        }
        ...
        while (true) {
            ...
            if (batch.length < BATCH || !deletedAny)
                break;
        }
        while (true) {
            const batch = args.tenantId
...
export const runScheduledCleanup = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        while (true) {
            ...
            if (effectiveRoles.length < BATCH_SIZE || !deletedAny)
                break;
        }
        while (true) {
            ...
            if (effectivePermissions.length < BATCH_SIZE || !deletedAny)
                break;
        }
        return {
            expiredRoleAssignments,
...
function getOptionalEnvNumber(name) {
    try {
        const g = globalThis;
...
export const runAuditRetentionCleanup = mutation({
    ...
    handler: async (ctx, args) => {
        const maxAgeDays = args.maxAgeDays ?? getOptionalEnvNumber("AUDIT_RETENTION_DAYS");
        const maxEntries = args.maxEntries ?? getOptionalEnvNumber("AUDIT_RETENTION_MAX_ENTRIES");
        let deletedByAge = 0;
            ...
            if (args.tenantId) {
                while (true) {
                    const batch = await ctx.db
                        .query("auditLog")
                        .withIndex("by_tenant_timestamp", (q) => q.eq("tenantId", args.tenantId).lt("timestamp", cutoff))
                        .order("asc")
                        .take(BATCH_SIZE);
                    if (batch.length === 0)
                        break;
                ...
                while (true) {
                    ...
                    if (batch.length < BATCH_SIZE || !deletedAny)
                        break;
                }
            }
        }
        if (maxEntries !== undefined && maxEntries > 0) {
            if (args.tenantId) {
                ...
                const keepEntries = await ctx.db
                    .query("auditLog")
                    .withIndex("by_tenant_timestamp", (q) => q.eq("tenantId", args.tenantId))
                    .order("desc")
                    .take(maxEntries);
                if (keepEntries.length === maxEntries) {
                    // There may be excess entries; delete anything not in the keep set
                    const keepIds = new Set(keepEntries.map((e) => e._id.toString()));
                    while (true) {
                        const batch = await ctx.db
                            .query("auditLog")
                            .withIndex("by_tenant_timestamp", (q) => q.eq("tenantId", args.tenantId))
