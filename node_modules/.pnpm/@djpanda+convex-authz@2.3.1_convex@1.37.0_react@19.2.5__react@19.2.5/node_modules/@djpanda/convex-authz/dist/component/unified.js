...
export const checkPermission = query({
    ...
    handler: async (ctx, args) => {
        ...
        const exact = await ctx.db
            ...
            .withIndex("by_tenant_user_permission_scope", (q) => q
        ...
        if (scopeKey !== "global") {
            const globalRows = await ctx.db
                .query("effectivePermissions")
                .withIndex("by_tenant_user_scope", (q) => q.eq("tenantId", args.tenantId)
                .eq("userId", args.userId)
                .eq("scopeKey", "global"))
                .take(4000);
        ...
        if (exact && !isExpired(exact.expiresAt) && exact.effect === "allow") {
            ...
            const rows = await ctx.db
                .query("effectivePermissions")
                .withIndex("by_tenant_user_scope", (q) => q
                .eq("tenantId", args.tenantId)
                .eq("userId", args.userId)
                .eq("scopeKey", scopeKey))
                .take(4000);
...
export const assignRoleUnified = mutation({
    args: {
        ...
        policyClassifications: v.optional(v.record(v.string(), v.union(v.null(), v.literal("allow"), v.literal("deny"), v.literal("deferred")))),
    },
    returns: v.string(),
    handler: async (ctx, args) => {
        const now = Date.now();
        ...
        const existing = await ctx.db
            ...
            .withIndex("by_tenant_user_and_role", (q) => q
            .eq("tenantId", args.tenantId)
            .eq("userId", args.userId)
            .eq("role", args.role))
            .take(100);
            ...
            if (scopeEquals(row.scope, args.scope) && !isExpired(row.expiresAt)) {
                ...
                if (shouldExtend && args.expiresAt !== row.expiresAt) {
                    ...
                    const effRole = await ctx.db
                        ...
                        .withIndex("by_tenant_user_role_scope", (q) => q.eq("tenantId", args.tenantId)
                        .eq("userId", args.userId)
                        .eq("role", args.role)
                        .eq("scopeKey", scopeKey))
                        .unique();
                    ...
                    for (const permission of args.rolePermissions) {
                        const effPerm = await ctx.db
                            .query("effectivePermissions")
                            .withIndex("by_tenant_user_permission_scope", (q) => q.eq("tenantId", args.tenantId)
                            .eq("userId", args.userId)
        ...
        const existingEffectiveRole = await ctx.db
            .query("effectiveRoles")
            .withIndex("by_tenant_user_role_scope", (q) => q
            .eq("tenantId", args.tenantId)
            .eq("userId", args.userId)
            .eq("role", args.role)
            .eq("scopeKey", scopeKey))
        ...
        for (const permission of args.rolePermissions) {
            ...
            if (existingPerm) {
                ...
                const patchData = { sources, updatedAt: now };
...
export const revokeRoleUnified = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const assignments = await ctx.db
            ...
            .eq("role", args.role))
            .take(100);
        const assignment = assignments.find((row) => scopeEquals(row.scope, args.scope));
        if (!assignment) {
            return false;
        }
        ...
        for (const permission of args.rolePermissions) {
            ...
            if (!effectivePerm)
                continue;
            const updatedSources = effectivePerm.sources.filter((s) => s !== args.role);
...
export const grantPermissionUnified = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingOverrides = await ctx.db
            ...
            .withIndex("by_tenant_user_and_permission", (q) => q
            .eq("tenantId", args.tenantId)
            .eq("userId", args.userId)
            .eq("permission", args.permission))
            .take(100);
        const existingOverride = existingOverrides.find((row) => scopeEquals(row.scope, args.scope));
        let overrideId;
        if (existingOverride) {
...
export const setAttributeWithRecompute = mutation({
    args: {
        ...
        policyReEvaluations: v.optional(v.record(v.string(), v.union(v.literal("allow"), v.literal("deny")))),
    },
    ...
    handler: async (ctx, args) => {
        ...
        const existingAttr = await ctx.db
            ...
            .withIndex("by_tenant_user_and_key", (q) => q
            .eq("tenantId", args.tenantId)
            .eq("userId", args.userId)
            .eq("key", args.key))
            .unique();
        ...
        if (args.policyReEvaluations) {
            ...
            const allEffectivePerms = await ctx.db
                .query("effectivePermissions")
                .withIndex("by_tenant_user", (q) => q
                .eq("tenantId", args.tenantId)
                .eq("userId", args.userId))
                .take(4000);
            for (const [permission, newResult] of Object.entries(args.policyReEvaluations)) {
                const matchingPerms = allEffectivePerms.filter((p) => p.permission === permission);
    ...
    handler: async (ctx, args) => {
        ...
        if (existing) {
            ...
            const existingEffective = await ctx.db
                ...
                .withIndex("by_tenant_subject_relation_object", (q) => q.eq("tenantId", args.tenantId)
...
export const removeRelationUnified = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const inherited = await ctx.db
            .query("effectiveRelationships")
            .withIndex("by_tenant_inherited_from", (q) => q
            .eq("tenantId", args.tenantId)
...
export const assignRolesUnified = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.roles.length > MAX_BULK_ROLES) {
            throw new Error(`roles must not exceed ${MAX_BULK_ROLES} items (got ${args.roles.length})`);
        }
        const now = Date.now();
        const assignmentIds = [];
        let assigned = 0;
    ...
    handler: async (ctx, args) => {
        ...
        for (const item of args.roles) {
            ...
            const assignments = await ctx.db
                ...
                .eq("role", item.role))
                .take(100);
            const assignment = assignments.find((row) => scopeEquals(row.scope, item.scope));
            if (!assignment) {
                continue;
            }
            // 3. Delete from roleAssignments
            ...
            for (const permission of permissions) {
                ...
                if (!effectivePerm)
                    continue;
                const updatedSources = effectivePerm.sources.filter((s) => s !== item.role);
                if (updatedSources.length === 0 && !effectivePerm.directGrant && !effectivePerm.directDeny) {
                    // No more sources, no direct grant, no direct deny — delete the row
                    await ctx.db.delete(effectivePerm._id);
                }
    ...
    handler: async (ctx, args) => {
        ...
        for (const assignment of assignments) {
            ...
            if (args.scope) {
                if (!assignment.scope)
                    continue;
                if (assignment.scope.type !== args.scope.type ||
            ...
            for (const permission of permissions) {
                ...
                if (!effectivePerm)
                    continue;
                const updatedSources = effectivePerm.sources.filter((s) => s !== assignment.role);
                if (updatedSources.length === 0 && !effectivePerm.directGrant && !effectivePerm.directDeny) {
                    await ctx.db.delete(effectivePerm._id);
                }
                else if (updatedSources.length !== effectivePerm.sources.length) {
...
export const syncRoleAction = action({
    ...
    handler: async (ctx, args) => {
        ...
        const seen = new Set();
        while (true) {
            const batch = await ctx.runQuery(api.unified.listUserIdsWithRole, {
                tenantId: args.tenantId,
                role: args.role,
