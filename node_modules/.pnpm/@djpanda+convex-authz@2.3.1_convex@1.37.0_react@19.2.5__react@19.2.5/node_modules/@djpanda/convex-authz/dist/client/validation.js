...
export function validateTenantId(tenantId) {
    ...
    if (tenantId.length > MAX_TENANT_ID_LENGTH) {
        throw new Error(`tenantId must not exceed ${MAX_TENANT_ID_LENGTH} characters`);
    }
}
/**
 ...
 */
export function validateRole(role, knownRoles) {
    ...
    if (knownRoles !== undefined) {
        const set = knownRoles instanceof Set
            ? knownRoles
 ...
 */
export function validateOptionalExpiresAt(expiresAt) {
 ...
 */
export function validateAttributeKey(key) {
 ...
 */
export function validatePermissions(permissions, maxLength = MAX_BULK_PERMISSIONS) {
    ...
    if (permissions.length > maxLength) {
        throw new Error(`permissions must not exceed ${maxLength} items (got ${permissions.length})`);
    }
 ...
 */
export function validateRoles(roles, knownRoles, maxLength = MAX_BULK_ROLES) {
    ...
    if (roles.length > maxLength) {
        throw new Error(`roles must not exceed ${maxLength} items (got ${roles.length})`);
    }
 ...
 */
export function validateRoleAssignItems(roles, knownRoles, maxLength = MAX_BULK_ROLES) {
    if (!Array.isArray(roles)) {
        throw new Error("roles must be an array");
    }
    if (roles.length === 0) {
    ...
    for (const item of roles) {
        ...
        if (item.expiresAt !== undefined)
            validateOptionalExpiresAt(item.expiresAt);
    }
}
/**
 * Validate relation args for hasRelation, addRelation, removeRelation: all non-empty strings.
 */
export function validateRelationArgs(subjectType, subjectId, relation, objectType, objectId) {
