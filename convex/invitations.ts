    ...
    if (!skipIdentifier && invitation.inviteeIdentifier) {
      if (!args.acceptingUserIdentifier) {
        throw new ConvexError({
          code: "FORBIDDEN",
          message:
            "acceptingUserIdentifier is required to accept this invitation",
        });
      }
      const normalize = (s: string) => s.trim().toLowerCase();
      if (
        normalize(args.acceptingUserIdentifier) !==
        normalize(invitation.inviteeIdentifier)
      ) {
        throw new ConvexError({
          code: "FORBIDDEN",
