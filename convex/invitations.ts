...
export const listInvitations = query({
  args: {
    ...
    sortBy: v.optional(
      v.union(v.literal('inviteeIdentifier'), v.literal('expiresAt'), v.literal('createdAt'))
    ),
    sortOrder: v.optional(v.union(v.literal('asc'), v.literal('desc'))),
    paginationOpts: v.optional(paginationOptsValidator),
  },
  returns: v.union(v.array(invitationShape), paginationResultValidator(invitationShape)),
...
export const inviteMember = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existing = await ctx.db
      ...
      .withIndex('by_org_invitee_and_status', (q) =>
        q
          ...
          .eq('inviteeIdentifier', normalizedIdentifier)
          .eq('status', 'pending')
      )
      .first()
    if (existing) {
    ...
    const invitationId = await ctx.db.insert('invitations', {
      ...
      role: args.role,
      teamId: args.teamId ? (args.teamId as Id<'teams'>) : undefined,
      inviterId: args.userId as Id<'users'>,
...
export const acceptInvitation = mutation({
  ...
  handler: async (ctx, args) => {
    const skipIdentifier =
      args.skipIdentifierCheck === true || args.trustedSkipToken === TRUSTED_WRAPPER_SKIP_SENTINEL
    ...
    const existingMember = await ctx.db
      ...
      .withIndex('by_organization_and_user', (q) =>
        q
          ...
          .eq('userId', args.acceptingUserId as Id<'users'>)
      )
      .unique()
    if (existingMember) {
      throw new ConvexError({
    ...
    await ctx.db.insert('members', {
      ...
      userId: args.acceptingUserId as Id<'users'>,
      role: invitation.role,
      status: 'active',
      joinedAt: Date.now(),
    })
    if (invitation.teamId) {
      ...
      const existingTm = await ctx.db
        ...
        .withIndex('by_team_and_user', (q) =>
          q
            .eq('teamId', invitation.teamId as Id<'teams'>)
            .eq('userId', args.acceptingUserId as Id<'users'>)
        )
        .unique()
      if (!existingTm) {
...
export const resendInvitation = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const callerMember = await ctx.db
      .query('members')
      .withIndex('by_organization_and_user', (q) =>
        q.eq('organizationId', invitation.organizationId).eq('userId', args.userId as Id<'users'>)
      )
      .unique()
  ...
  handler: async (ctx, args) => {
    if (args.invitations.length > 100) {
      throw new ConvexError({
        ...
        message: 'Cannot invite more than 100 members at once',
      })
    }
    const orgId = args.organizationId as Id<'organizations'>
    ...
    for (const inv of args.invitations) {
      ...
      try {
        ...
        const invitationId = await ctx.db.insert('invitations', {
          ...
          role: inv.role,
          teamId: inv.teamId ? (inv.teamId as Id<'teams'>) : undefined,
          inviterId: args.userId as Id<'users'>,
          inviterName: args.inviterName,
          message: inv.message,
          status: 'pending',
