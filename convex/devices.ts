  ...
  handler: async (ctx, args) => {
    ...
    if (!device) {
      return {
        ...
        message: 'Ce numéro de série ne correspond à aucun TPE enregistré.',
      }
    }
    ...
    if (!device.assignedTo) {
      return {
        ...
        message: "Ce TPE n'est assigné à aucun agent. Contactez votre responsable.",
      }
    }
    ...
    if (device.assignedTo !== user._id) {
      return {
        valid: false,
        reason: 'wrong_agent',
        message: "Ce TPE n'est pas celui qui vous a été assigné. Veuillez scanner le bon appareil.",
      }
    }
