...
export type AgentRole = 'field_agent' | 'supervisor' | 'accountant' | 'admin' | 'it_admin'
...
async function assertCanManage(ctx: any, branchId: any, callerSubject: string): Promise<void> {
  const roles = await authz.withTenant(branchId).getUserRoles(ctx, callerSubject)
  ...
  handler: async (ctx, args) => {
    ...
    return Promise.all(
      users.map(async (u: any) => {
        ...
        const online = device ? new Date(device.lastSync ?? 0).getTime() > fiveMinutesAgo : false
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (duplicate) {
      throw new Error('Un agent avec ce numéro ou cet email existe déjà dans cette agence')
    }
    ...
    await authz.withTenant(args.branchId).assignRole(ctx, `invited|${userId}`, args.role)
...
export const linkAgentAccount = mutation({
  ...
  handler: async (ctx) => {
    ...
    if (!agentRecord) {
      throw new Error('Aucun compte trouvé pour ce numéro. Contactez votre gérant.')
    }
    ...
    const oldRoles = await authz.withTenant(agentRecord.branchId).getUserRoles(ctx, oldToken)
    const role = oldRoles[0]?.role
    if (role) {
      await authz.withTenant(agentRecord.branchId).assignRole(ctx, identity.subject, role)
    }
...
export const changePassword = action({
  ...
  handler: async (ctx, args): Promise<void> => {
    ...
    const phoneNumber: string | null = await ctx.runQuery(internal.users.getPhoneNumberById, {
      userId,
    })
