...
export const listByBranch = query({
  ...
  handler: async (ctx, args) => {
    ...
    await authz.withTenant(targetBranchId).require(ctx, identity.subject, 'customers:view')
...
export const update = mutation({
  args: {
    ...
    idNumber: v.optional(v.string()),
    status: v.optional(
      v.union(v.literal('prospect'), v.literal('verified'), v.literal('rejected'))
    ),
    balance: v.optional(v.number()),
    metadata: metadataValidator,
  },
  handler: async (ctx, { id, ...fields }) => {
    ...
    await authz.withTenant(agent.branchId).require(ctx, identity.subject, 'customers:update')
    ...
    if (fields.phoneNumber !== undefined) patch.phoneNumber = fields.phoneNumber.trim()
