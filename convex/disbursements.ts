...
export const listPending = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!user || user.branchId !== args.branchId) {
      throw new Error(
        "Unauthorized: Cannot view disbursements for this branch",
      );
    }
    return ctx.db
      .query("disbursements")
...
export const listHistory = query({
  ...
  handler: async (ctx, args) => {
    ...
    return [...approved, ...rejected, ...executed].sort(
...
export const rejectDisbursement = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (record.branchId !== user.branchId) {
      throw new Error(
        "Unauthorized: Cannot reject disbursements from another branch",
      );
    }
