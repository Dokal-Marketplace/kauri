...
export function EmptyState(props) {
        ...
        {filterTweaks && filterTweaks.length > 0 && (
          ...
          >
            {filterTweaks.map((t, i) => (
              ...
              >
                <span style={{ color: 'var(--tofee-muted)', flexShrink: 0, display: 'flex' }}>
                  {t.icon}
                </span>
                <span style={{ flex: 1, color: 'var(--tofee-ink)', fontWeight: 500 }}>
                  {t.label}
                </span>
                {t.kbd && (
                  <kbd
  ...
  if (props.variant === 'celebration') {
    const { emoji, celebrationStyle, eyebrow, eyebrowColor, title, description, actions, tips } =
      props
    return (
      <div
        style={{
      ...
      >
        ...
        {eyebrow && (
          <div style={{ ...T.eyebrow, color: eyebrowColor ?? 'var(--tofee-success)' }}>
            {eyebrow}
          </div>
        )}
        ...
        <div style={{ width: 140, height: 140, marginBottom: 22, position: 'relative' }}>
          <div
            style={{
              position: 'absolute',
              inset: 0,
              background: 'radial-gradient(circle, var(--tofee-orange-glow) 0%, transparent 70%)',
              borderRadius: '50%',
            }}
          />
          <div
...
/* ── Suggest chip ─────────────────────────────────────────────────── */
export function SuggestChip({ children, onClick }) {
  return (
    <button
      onClick={onClick}
      style={{
...
export function EmptyInline({ message, action }) {
  return (
    <div
      style={{
        padding: '28px 20px',
