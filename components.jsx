// Kauri dashboard — main app component
...
function Topbar({ online, setOnline, onNewClient }) {
  return (
    <div className="topbar">
      <div>
        <div className="crumbs"><strong>Kauri</strong> &nbsp;/&nbsp; Tableau de bord</div>
        <h1 className="h1">Bonsoir, Djibril</h1>
        <div className="h1-sub">Mardi 5 mai 2026 · Agence Bobo-Dioulasso · 6 agents en service</div>
      </div>
      <div className="topbar-right">
        <div className="search" style={{ width: 240 }}>
          <I.Search /><input placeholder="Rechercher un client, transaction…" />
          <span style={{ fontSize: 11, color: "var(--ink-4)", border: "1px solid var(--border)", padding: "1px 5px", borderRadius: 4 }}>⌘K</span>
        </div>
        ...
        <button className="btn brand" onClick={onNewClient}><I.Plus size={14} stroke="white"/>Nouveau client</button>
      </div>
    </div>
  );
}
      ...
      <div className="chart-wrap">
        <svg viewBox={`0 0 ${w} ${h}`} width="100%" style={{ display: "block" }}>
          ...
          {data.map((d, i) => {
            ...
            return (
              <g key={d.m}>
                <rect x={x} y={h - pad.b - outH} width={bw} height={outH}
                      fill="oklch(0.85 0.008 70)" rx="3" />
                <rect x={x} y={h - pad.b - outH - inH} width={bw} height={inH}
                      fill="var(--brand)" rx="3" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, padding: "4px 14px 16px" }}>
        {actions.map((a, i) => (
          <button key={i} className="btn" style={{
            ...
            }}>{React.cloneElement(a.ic, { size: 14 })}</span>
...
window.KauriComponents = {
  Sparkline, Sidebar, Topbar, KPI, ClientsCard, TransactionsCard, VolumeChart, GoalsCard, ActivityCard, QuickActionsCard, fmt
};
